...
class Agent:
    ...
    def minimax(self, node, depth, isMaximizingPlayer, alpha, beta):
        if depth == 0 or self.game_over(node, self._turn):
            blue_tokens = {(b_x, b_y): (b_c, b_v) for (b_x, b_y), (b_c, b_v) in node.items() if b_c == PlayerColor.BLUE}
            red_tokens = {(r_x, r_y): (r_c, r_v) for (r_x, r_y), (r_c, r_v) in node.items() if r_c == PlayerColor.RED}
            return power_difference_heuristic(red_tokens, blue_tokens, self._color), None
